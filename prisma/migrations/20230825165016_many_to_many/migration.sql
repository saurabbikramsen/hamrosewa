/*
  Warnings:

  - You are about to drop the column `serviceId` on the `interaction` table. All the data in the column will be lost.
  - You are about to drop the column `userId` on the `interaction` table. All the data in the column will be lost.

*/
-- DropForeignKey
ALTER TABLE `interaction` DROP FOREIGN KEY `interaction_serviceId_fkey`;

-- DropForeignKey
ALTER TABLE `interaction` DROP FOREIGN KEY `interaction_userId_fkey`;

-- AlterTable
ALTER TABLE `interaction` DROP COLUMN `serviceId`,
    DROP COLUMN `userId`;

-- CreateTable
CREATE TABLE `_interactionTouser` (
    `A` INTEGER NOT NULL,
    `B` INTEGER NOT NULL,

    UNIQUE INDEX `_interactionTouser_AB_unique`(`A`, `B`),
    INDEX `_interactionTouser_B_index`(`B`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- CreateTable
CREATE TABLE `_interactionToservices` (
    `A` INTEGER NOT NULL,
    `B` INTEGER NOT NULL,

    UNIQUE INDEX `_interactionToservices_AB_unique`(`A`, `B`),
    INDEX `_interactionToservices_B_index`(`B`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- AddForeignKey
ALTER TABLE `_interactionTouser` ADD CONSTRAINT `_interactionTouser_A_fkey` FOREIGN KEY (`A`) REFERENCES `interaction`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `_interactionTouser` ADD CONSTRAINT `_interactionTouser_B_fkey` FOREIGN KEY (`B`) REFERENCES `user`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `_interactionToservices` ADD CONSTRAINT `_interactionToservices_A_fkey` FOREIGN KEY (`A`) REFERENCES `interaction`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `_interactionToservices` ADD CONSTRAINT `_interactionToservices_B_fkey` FOREIGN KEY (`B`) REFERENCES `services`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
